...
The more interesting result, is the level of sparsity `PAdam` obtained. In this specific examples, `PAdam` pushed $79.7\%$ of the weights to 0,  whereas `AdamW` pushed only about $6.3\%$ below $10^{-9}$. This is a significant difference, which possibly could lead to reduced inference time, if treated correctly. Below we show the CDF of the absolute values of the weights, which shows nicely that for `PAdam`, practically all the non-vanishing weights are larger than $(p\lambda)^{1/(2-p)}\simeq6.6\times 10^{-3}$, which is the scale below which we expect to see asupression (c.f. [Eq.[3]](#eq_GD_update)).

![Desktop View](/assets/images/fashion_padam_cdf.png){: width="600" height="589" }
_Cumulative Distribution Function of the magnitude of weights for both `AdamW` and `PAdam`. The gray dashed line represent the value of $(p\lambda)^{1/(2-p)}$, below which `PAdam` is expected to suppress the weights._
